...
class errorTransaction implements WithMultipleSheets
{
    ...
    public function array(): array
    {
        $data[] = [
            ...
            [
                ...
                'Penjelasan: ID Umat wajib diisi dengan ID yang valid dari tabel `umats`. Pastikan ID Umat yang dimasukkan ada dalam database dan dapat diverifikasi.harus diisi dengan awalan , (comma) dan berformat Text!!',
        ...
        if (!empty($failedRows)) {
            foreach ($failedRows as $row) {
                $data[] = [$row['status'] ?? '', $row['judul'] ?? '', $row['id_romo'] ?? '', $row['id_seksi'] ?? '', $row['id_acara'] ?? '', $row['id_doa'] ?? '', $row['id_umat'] ?? '', $row['jadwal_transaction'] ?? '', $row['jam_transaction'] ?? '', $row['deskripsi_transaksi'] ?? ''];
            }
        }
        return $data;
    }
